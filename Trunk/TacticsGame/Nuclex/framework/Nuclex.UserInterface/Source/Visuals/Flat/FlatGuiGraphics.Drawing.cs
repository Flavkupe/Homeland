...
namespace Nuclex.UserInterface.Visuals.Flat {
  ...
  partial class FlatGuiGraphics {
    ...
    public void DrawElement(string frameName, RectangleF bounds, Texture2D myTexture, Rectangle? clip)
    {
        this.DrawElement(frameName, bounds, myTexture, clip, Color.White);
    }

    // following method added by CSP:
    public void DrawElement(string frameName, RectangleF bounds, Texture2D myTexture, Rectangle? clip, Color color)
    {
        Frame frame = lookupFrame(frameName);
        ...
        // that needs to be blit somewhere into the element to form the element's
        // visual representation step by step.
        for (int index = 4; index <= 4; ++index)  //for (int index = 0; index < frame.Regions.Length; ++index)
        {
            //Rectangle destinationRegion = calculateDestinationRectangle(ref bounds, ref frame.Regions[index].DestinationRegion);
            Rectangle destinationRegion = new Rectangle((int)bounds.Left, (int)bounds.Top, (int)bounds.Width, (int)bounds.Height);
    ...
    public void DrawString(string frameName, RectangleF bounds, string text, Color color, SpriteFont font = null)
    {
        Frame frame = lookupFrame(frameName);
        ...
        {
            this.spriteBatch.DrawString(
                font == null ? frame.Texts[index].Font : font,
              text,
              positionText(ref frame.Texts[index], bounds, text),
              color
            );
