...
{
    public partial class UnitSelectDialog : ModalDialogControl
    {
        ...
        public static UnitSelectDialog CreateDialog(List<Unit> list, Unit selectedUnit = null, bool showAutomatically = true) 
        {
            UnitSelectDialog newDialog = new UnitSelectDialog();
            ...
            newDialog.AffectsOrdering = true;
        ...
        private void HandleHireButtonPressed(object sender, EventArgs e)
        {
            ...
            {
                MessageDialog prompt = MessageDialog.CreateDialog(string.Format("It will cost {0} to hire {1}. Pay?", cost, this.selectedUnit.Name));
                prompt.YesOrNoPromptMode = true;
                prompt.ButtonResult += this.HandleHirePromptButtonResult;
            }
            else
            {
                MessageDialog prompt = MessageDialog.CreateDialog(string.Format("It will cost {0} to hire {1}. You cannot afford it.", cost, this.selectedUnit.Name));
            }
        }
        ...
        private void HandleHirePromptButtonResult(object sender, EventArgsEx<MessageDialogResult> e)
        {
            if (e.Value == MessageDialogResult.Yes)
            {
                if (this.HiredUnit != null)
                {
                    ...
                    MessageDialog prompt = MessageDialog.CreateDialog(string.Format("Unit {0} was hired and will begin working next turn.", this.selectedUnit.Name));
        ...
        public void SetUnits(List<Unit> units)
        {
            ...
            {
                Texture2D icon = unit.GetEntityIcon();
                TooltipButtonAndTextControl newButton = new TooltipButtonAndTextControl(icon, unit.Name, 200);
                newButton.TooltipText = unit.Name;
                newButton.Tag = unit;
    ...
    public partial class UnitSelectDialog
    {
