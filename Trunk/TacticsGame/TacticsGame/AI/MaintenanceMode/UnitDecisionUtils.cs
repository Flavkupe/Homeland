...
namespace TacticsGame.AI.MaintenanceMode
{
    ...
    {
        public static bool DecisionRequiresBuilding(Decision decision)
        {
            return decision == Decision.Buy || decision == Decision.Sell;
        }

        public static bool DecisionRequiresVisitor(Decision decision)
        {
            return decision == Decision.Buy || decision == Decision.Sell;
        }
        ...
        /// </summary>
        public static int DetermineWillingnessToSellItem(IMakeDecisions unit, Item item)
        {
            int propensity = 0;
            int itemCount = unit.Inventory.GetItemCount(item);
            //propensity -= unit.ItemTypePreference.GetValueOrZero<ItemType>(item.Stats.Type); // Want to keep preferred item types
            propensity -= unit.ItemPreference.GetValueOrZero<string>(item.ObjectName); // Want to keep preferred items
        ...
        /// How willing the unit is to part with the item.
        /// </summary>
        public static int DetermineWillingnessToBuyItem(IMakeDecisions unit, Item item)
        {
            ...
            int propensity = 0;
            int itemCount = unit.Inventory.GetItemCount(item);
            propensity += unit.ItemTypePreference.GetValueOrZero<ItemType>(item.Stats.Type); // Want to buy preferred item types
            propensity += unit.ItemPreference.GetValueOrZero<string>(item.ObjectName); // Want to buy preferred items            
        ...
        public static int MakeBid(IMakeDecisions unit, Item item, bool sellingBid)
        {                        
            ...
            bid = sellingBid ? Math.Max(1, bid + Utilities.GetRangePercent(bid, 0, unit.PriceMarkupRange)) :
                               Math.Max(1, bid - Utilities.GetRangePercent(bid, 0, unit.PriceMarkupRange));
        ...
        public static int MakeAppraisal(IMakeDecisions unit, Item item, int boost = 0) 
        {
        ...
        /// </summary>
        public static int ImproveBid(IMakeDecisions unit, Item item, int currentBid)
        {
            ...
            int reassessment = UnitDecisionUtils.MakeAppraisal(unit, item, assessmentBoost); // Try to guess the price again with more info.
        ...
        public static bool UnitWantsToBuyItem(IMakeDecisions unitBuying, IMakeDecisions unitSelling, Item item, int ask)
        {
            ...
            int opposingBid = UnitDecisionUtils.MakeBid(unitBuying, item, false);
            ...
            leeway += Math.Max(unitBuying.ItemPreference.GetValueOrZero<string>(item.ObjectName),
                               unitBuying.ItemTypePreference.GetValueOrZero<ItemType>(item.Stats.Type)); // Get max of preference. Add directly.
